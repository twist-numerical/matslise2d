...
#define MATSLISE2D_INTEGRATE_delta (2*MATSLISE_HMAX_delta)
#define MATSLISE2D_INTEGRATE_eta_rows (MATSLISE_ETA_delta*MATSLISE_ETA_delta) // ((MATSLISE_HMAX_delta/2+1)*(MATSLISE_HMAX_delta/2 + 2)/2)
...
Eigen::Array<Scalar, MATSLISE2D_INTEGRATE_eta_rows, Eigen::Dynamic> etaProduct(
    ...
    Eigen::Array<Scalar, MATSLISE2D_INTEGRATE_eta_rows, Eigen::Dynamic> uv
            = Eigen::Array<Scalar, MATSLISE2D_INTEGRATE_eta_rows, Eigen::Dynamic>
            ::Zero(MATSLISE2D_INTEGRATE_eta_rows, MATSLISE2D_INTEGRATE_delta);

    for (int i = 0; i < MATSLISE_ETA_delta; ++i)
        for (int j = 0; j < MATSLISE_ETA_delta; ++j) {
            Eigen::Index ij = ETA_index(i, j);
            if (ij >= MATSLISE2D_INTEGRATE_eta_rows) break;
            for (int ni = 0; ni < MATSLISE_HMAX_delta; ++ni)
                for (int nj = 0; nj < MATSLISE_HMAX_delta && ni + nj < MATSLISE2D_INTEGRATE_delta; ++nj)
...
template<typename Scalar>
Eigen::Array<Scalar, MATSLISE2D_INTEGRATE_delta, 1> integrateTaylorTaylor(
        const Scalar &delta,
        const Scalar &dZ1, const Scalar *eta1,
        const Scalar &dZ2, const Scalar *eta2,
        int degree
) {
    Eigen::Array<Scalar, MATSLISE2D_INTEGRATE_delta, 1> result
            = Eigen::Array<Scalar, MATSLISE2D_INTEGRATE_delta, 1>::Zero();
    Scalar x1n = 1;
    for (int i = 0; i <= degree; ++i) {
        Scalar x2n = 1;
        for (int j = 0; j <= degree; ++j) {
            for (int k = 0; k < MATSLISE2D_INTEGRATE_delta; ++k)
...
Eigen::Array<Scalar, MATSLISE2D_INTEGRATE_delta, 2> integrateEtaTaylor(
        const Scalar &delta,
        const Scalar &dZ1, const Scalar &dZ2, const Scalar *taylor) {
    Eigen::Array<Scalar, MATSLISE2D_INTEGRATE_delta, 2> _I
            = Eigen::Array<Scalar, MATSLISE2D_INTEGRATE_delta, 2>::Zero();
    ...
    Scalar di = powInt(delta, MATSLISE2D_INTEGRATE_delta + 2 * degree - 1);
    for (int i = MATSLISE2D_INTEGRATE_delta + 2 * degree - 2; i > 0; --i) {
        ...
        for (int k = std::max(0, i - MATSLISE2D_INTEGRATE_delta + 1); k <= degree && 2 * k <= i; ++k) {
...
eta_integrals(const Scalar &delta, const Scalar &dZ1, const Scalar &dZ2) {
    Eigen::Array<Scalar, MATSLISE2D_INTEGRATE_eta_rows, Eigen::Dynamic> _I
            = Eigen::Array<Scalar, MATSLISE2D_INTEGRATE_eta_rows, Eigen::Dynamic>
            ::Zero(MATSLISE2D_INTEGRATE_eta_rows, MATSLISE2D_INTEGRATE_delta);
    ...
    if (abs(Z1) < 0.5 && abs(Z2) < 0.5) {
        Eigen::Array<Eigen::Array<Scalar, MATSLISE2D_INTEGRATE_delta, 1>, 2, 2> step;
        ...
        for (int i = 8; i >= 0; --i) {
            {
                for (int j = 0; j < 2; ++j) {
                    Eigen::Array<Scalar, MATSLISE2D_INTEGRATE_delta, 1> curr = step(0, j);
                    step(0, j)(MATSLISE2D_INTEGRATE_delta - 1) = 0;
                    step(0, j)(MATSLISE2D_INTEGRATE_delta - 2) = 0;
                    step(0, j).template topRows<MATSLISE2D_INTEGRATE_delta - 2>() =
                            dZ1 * step(1, j).template bottomRows<MATSLISE2D_INTEGRATE_delta - 2>()
                            + (2 * i + 1) * curr.template topRows<MATSLISE2D_INTEGRATE_delta - 2>();
                    step(1, j) = curr;
                }
            }
            Eigen::Array<Eigen::Array<Scalar, MATSLISE2D_INTEGRATE_delta, 1>, 1, 2> stepUp = step.row(0);
            for (int j = 8; j >= 0; --j) {
                Eigen::Array<Scalar, MATSLISE2D_INTEGRATE_delta, 1> curr = stepUp(0);
                stepUp(0)(MATSLISE2D_INTEGRATE_delta - 1) = 0;
                stepUp(0)(MATSLISE2D_INTEGRATE_delta - 2) = 0;
                stepUp(0).template topRows<MATSLISE2D_INTEGRATE_delta - 2>() =
                        dZ2 * stepUp(1).template bottomRows<MATSLISE2D_INTEGRATE_delta - 2>()
                        + (2 * j + 1) * curr.template topRows<MATSLISE2D_INTEGRATE_delta - 2>();
                stepUp(1) = curr;
                Eigen::Index ij = ETA_index(i, j);
                if (i < MATSLISE_ETA_delta && j < MATSLISE_ETA_delta && ij < MATSLISE2D_INTEGRATE_eta_rows)
                    _I.row(ij) = stepUp(0);
            }
        }
        return _I;
        ...
        for (int i = 8; i >= 0; --i) {
            ...
            for (int j = 0; j < MATSLISE_ETA_delta; ++j) {
                Eigen::Index ij = ETA_index(i, j);
                if (i < MATSLISE_ETA_delta && ij < MATSLISE2D_INTEGRATE_eta_rows)
                    _I.row(ij) = stepUp(0);
                Eigen::Array<Scalar, MATSLISE2D_INTEGRATE_delta, 1> curr = stepUp(1);
                stepUp(1)(0) = 0;
                stepUp(1)(1) = 0;
                stepUp(1).template tail<MATSLISE2D_INTEGRATE_delta - 2>()
                        = ((stepUp(0) - (2 * j + 1) * curr) / dZ2)
                        .template head<MATSLISE2D_INTEGRATE_delta - 2>();
                stepUp(0) = curr;
            }
        }
        ...
        for (int i = 0; i < MATSLISE_ETA_delta; ++i) {
            ...
            {
                for (int j = 0; j < 2; ++j) {
                    Eigen::Array<Scalar, MATSLISE2D_INTEGRATE_delta, 1> curr = step(1, j);
                    step(1, j)(0) = 0;
                    step(1, j)(1) = 0;
                    step(1, j).template tail<MATSLISE2D_INTEGRATE_delta - 2>()
        ...
        int lastN = 100;// MATSLISE2D_INTEGRATE_delta - 3;
        ...
        for (int n = lastN; n > 1; --n) {
            ...
            if (n - 1 < MATSLISE2D_INTEGRATE_delta)
                _I(ETA_index(0, 0), n - 1) = I00n;
            if (n + 1 < MATSLISE2D_INTEGRATE_delta)
                _I(ETA_index(1, 1), n + 1) = I11n;
            if (n < MATSLISE2D_INTEGRATE_delta) {
        ...
        for (int i = 0; i < MATSLISE_ETA_delta; ++i) {
            if (i > 1) {
                for (int j = 0; j < 2; ++j) {
                    Eigen::Index ij = ETA_index(i, j);
                    if (ij >= MATSLISE2D_INTEGRATE_eta_rows) break;
                    _I.row(ij).template tail<MATSLISE2D_INTEGRATE_delta - 2>()
